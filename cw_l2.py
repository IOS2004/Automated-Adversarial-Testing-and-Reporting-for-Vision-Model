...
#         input_var = torch.tensor(torch_arctanh(inputs),requires_grad=False)
        ...
        target_var = target_onehot.detach().clone().requires_grad_(False)
        
        modifier = torch.zeros(input_var.size()).float()
        
        modifier = torch.normal(mean=modifier, std = 1e-3)
        modifier = modifier.cuda()
        modifier_var = modifier.detach().clone().requires_grad_(True)
        ...
        for search_step  in range(self.bsearch_steps):
            ...
            scale_const_var = scale_const_tensor.detach().clone().requires_grad_(False)
...
#         criterion = nn.CrossEntropyLoss()
    ...
    print('hi')
    wagner = CarliniWagnerL2(img_tsor, model, targetLabel, 0.1, -1, 1, 0, 4, 500)
    ...
    # Save CW L2 adversarial image
    save_adversarial_image(adv_img_tsor, img_name, 'cw_l2', 'targeted')
    ...
    # For targeted attack: count MATCHES (when prediction equals target)
    if (int(targetLabel) == int(adv_pred)):
        targ_misclassifn_top1 += 1
    if (int(targetLabel) in top_labels.astype(int)):
        targ_misclassifn_top5 += 1
    
    # For misclassification: count MISMATCHES (when prediction differs from original)
...
print("\n" + "="*60)
print("CARLINI-WAGNER L2 ATTACK RESULTS")
print("="*60)
print(f"Total images tested: {batch_size}")
print(f"\nTargeted Attack:")
print(f"  Top-1 successes: {targ_misclassifn_top1} / {batch_size}")
print(f"  Top-1 failures: {batch_size - targ_misclassifn_top1} / {batch_size}")
print(f"  Top-1 success rate: {(targ_misclassifn_top1/batch_size)*100:.2f}%")
print(f"\n  Top-5 successes: {targ_misclassifn_top5} / {batch_size}")
print(f"  Top-5 success rate: {(targ_misclassifn_top5/batch_size)*100:.2f}%")
print(f"\nMisclassification (changed from original):")
print(f"  Top-1 misclassifications: {misclassifn_top1} / {batch_size}")
print(f"  Top-1 misclassification rate: {(misclassifn_top1/batch_size)*100:.2f}%")
print(f"  Top-5 misclassifications: {misclassifn_top5} / {batch_size}")
print(f"  Top-5 misclassification rate: {(misclassifn_top5/batch_size)*100:.2f}%")
print("="*60)
